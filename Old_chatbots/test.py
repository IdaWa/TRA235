...
# We'll try a different chunk size and overlap
...
st.title('Langchain Chatbot With Improved Retrieval & Prompting')
...
embedding_model = HuggingFaceEmbedding(model_name="sentence-transformers/multi-qa-mpnet-base-dot-v1")
...
# Create a more directive system prompt (context)
system_prompt = (
    "You are a helpful assistant that answers queries based on the provided documents. "
    "If the answer is not found explicitly in the documents, say 'I do not know'. "
    "First, extract the relevant details from the documents and then provide the final answer. "
    "Do not include any information not supported by the provided documents."
)

# Create query engines with a system prompt and instructions for reasoning
# Depending on the version of llama_index, you may pass system prompts differently.
manual_query_engine = manual_index.as_query_engine(
    llm=llm,
    additional_context=system_prompt,
    # If supported, you can try modes like QueryMode.EMBEDDING, or top_k to retrieve more documents:
...
if input_text.strip():
    ...
    # You can apply metadata filters if known. For instance, if the user
    # wants only from manuals:
    # manual_query_engine = manual_index.as_query_engine(
    #   llm=llm,
    #   additional_context=system_prompt,
    #   filters={"doc_type": "manual"}, # This depends on your version of llama_index
    #   response_mode="compact",
    #   similarity_top_k=5
    # )
